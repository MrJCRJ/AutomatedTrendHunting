export default function handler(req, res) {
  const html = [
    ...
    '<style>',
    ':root{--grad:linear-gradient(135deg,#667eea 0%,#764ba2 100%);--bg-panel:#1f1f2e;--accent:#667eea;--accent-hover:#5a6fd8;--danger:#d63939;--ok:#40c057;--warn:#ffd43b;--font:Segoe UI,Tahoma,sans-serif}',
    ...
    '<div id="stats-preview" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:14px;margin:10px 0 22px">',
    ...
    'function currentStore(){return localStorage.getItem(K)?localStorage:sessionStorage.getItem(K)?sessionStorage:null}',
    ...
    'async function exec(endpoint, btn){if(btn){btn.disabled=true;const original=btn.textContent;btn.textContent="...";try{const r=await fetch(endpoint,{method:"POST",headers:tokenHeader()});if(r.ok){loadStats();}else{console.warn("Falha op",endpoint);} }finally{btn.textContent=original;btn.disabled=false;}}}',
    'if(hasValid())unlock();',
